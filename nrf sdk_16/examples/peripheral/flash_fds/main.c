...
#define NUM_PAGES 4
//FS_SECTION_VARS_ADD( fs_config_t fs_config ) = { .callback = &fs_callback, .num_pages = NUM_PAGES, .page_order = 1 };
...
void uart_events_handler(app_uart_evt_t * p_event)
{
    switch (p_event->evt_type)
    {
        ...
        case APP_UART_TX_EMPTY:            //printf("hehe ----\r\n"); // out ADC result
            break;
...
//static uint32_t const * address_of_page(fs_config_t * fs_config, uint16_t page_num)
//{
...
static void fstorage_test(void)
{
		...
		fs_callback_flag = 1;
		ret = fs_store(&fs_config, fs_config.p_start_addr, &data, 1);      //Write data to memory address 0x0003F000. Check it with command: nrfjprog --memrd 0x0003F000 --n 16
		if (ret != FS_SUCCESS)
		{
				bsp_indication_set(BSP_INDICATE_FATAL_ERROR);
		}
